% adaptive_timer
function [ho_number, ho_delay, avg_throughput, bad_signal_time] = adaptive_timer(num_BS, L, BW, tot_time, time_unit, handoff_thd, ho_delay_wired, ho_wireless_bits, bad_signal_thd, shading_dev, pos_MS, dir_MS, spd_MS, para_P, para_M, para_Q, ada_mode, show_info, seed, movement_mode)
    % set random seed 
    rng(seed)
    ...
    % HO timer length arr (time unit)
    ho_timer_len_arr = timer_len_finder(d_arr, para_P, para_M, para_Q, ada_mode);     
    
    % Initialize HO timer array
    ho_timer_arr = ho_timer_len_arr;
    ...
    % the candidate to handoff to (-1 means no BS)
    ho_candidate = -1;
    ...
    for t = 1:time_unit:tot_time
        ...
        % max BS is not current BS
        if(max_id~=current_id)
            % if max_SINR> present BS SINR+(handoff_thd) -> handoff candidate
            if(max_SINR > SINR_arr(current_id)+handoff_thd)
                % max BS is already the ho_candidate
                if(max_id == ho_candidate)
                    % timer countdown
                    ho_timer_arr(ho_candidate) = ho_timer_arr(ho_candidate)-1;
    
                % max BS is not the ho_candidate 
                % (no candidate yet or candidate changed)
                else
                    % assign candidate
                    ho_candidate = max_id; 
                    % reset timer
        ...
        % timer drops to zero -> handoff
        if (ho_candidate>0)
            if(ho_timer_arr(ho_candidate) == 0)
                % log into history
                ...
                % update handoff candidate (no candidate)
                ho_candidate = -1;
